...
function M.setup(opt, checkpoint)
   ...
   if checkpoint then
      ...
      model = torch.load(modelPath):type('torch.CudaHalfTensor')
   elseif opt.retrain ~= 'none' then
      assert(paths.filep(opt.retrain), 'File not found: ' .. opt.retrain)
      print('Loading model from file: ' .. opt.retrain)
      model = torch.load(opt.retrain):type('torch.CudaHalfTensor')
   ...
   if opt.optnet then
      ...
      local sampleInput = torch.zeros(4,3,imsize,imsize):type('torch.CudaHalfTensor')
   ...
   if opt.resetClassifier and not checkpoint then
      ...
      model:add(linear:type('torch.CudaHalfTensor'))
   end
   ...
   if opt.nGPU > 1 then
      ...
      model = dpt:type('torch.CudaHalfTensor')
   end

   local criterion = nn.CrossEntropyCriterion():type('torch.CudaHalfTensor')
